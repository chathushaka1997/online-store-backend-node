...
export namespace ProductEp {
    export async function createProduct(req: Request, res: Response, next: NextFunction) {
      ...
      export async function updateProduct(req: Request, res: Response, next: NextFunction) {
        try {
          const productData = req.body;
          const updateProduct = await ProductDao.updateProduct(productData._id, productData);
          res.sendSuccess(updateProduct, "Product updated!", 200);
        } catch (error) {
          console.log("Error ----> " + error);
          res.sendError("Error " + error, 400);
        }
      ...
      export async function getProducts(req: Request, res: Response, next: NextFunction) {
        try {
          ...
          res.sendSuccess(products, "products retrireve!");
        } catch (error) {
          console.log("Error ----> " + error);
          res.sendError("Error " + error, 400);
        }
